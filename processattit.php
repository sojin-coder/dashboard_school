...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $check_sql = "SELECT id FROM attendantit
                      WHERE student_id = ? 
                      AND attendance_date = ? 
                      AND subject = ?
                      AND year = ?
    ...
    $sql = "INSERT INTO attendantit (student_id, attendance_date, subject, year, shift, status, note) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        error_log("SUCCESS: Inserted record ID: $record_id into attendantit");
