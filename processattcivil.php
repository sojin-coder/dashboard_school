...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $valid_subjects = ['Civil Engineering'];
    if (empty($subject)) {
    ...
    if (empty($errors)) {
        $check_sql = "SELECT id FROM attendantcivil
                      WHERE student_id = ? 
                      AND attendance_date = ? 
                      AND subject = ?
                      AND year = ?
    ...
    if (!empty($errors)) {
        ...
        header("Location: attcivil.php");
        exit();
    }
    
    // === INSERT RECORD (using correct table name: attendantit) ===
    $sql = "INSERT INTO attendantcivil (student_id, attendance_date, subject, year, shift, status, note) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        error_log("SUCCESS: Inserted record ID: $record_id into attendantcivil");
