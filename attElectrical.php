...
$sql = "SELECT * FROM attendantelectrical ORDER BY id DESC";
...
$sql_today = "SELECT COUNT(*) as total FROM attendantelectrical WHERE attendance_date = '$today'";
$result_today = mysqli_query($conn, $sql_today);
$today_count = mysqli_fetch_assoc($result_today)['total'];

// 2. Count by status for today
$sql_present = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$today' AND status = 'Active'";
$result_present = mysqli_query($conn, $sql_present);
$present_count = mysqli_fetch_assoc($result_present)['count'];

$sql_absent = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$today' AND status = 'A'";
$result_absent = mysqli_query($conn, $sql_absent);
$absent_count = mysqli_fetch_assoc($result_absent)['count'];

$sql_late = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$today' AND status = 'P'";
...
$sql_search = "SELECT COUNT(*) as total FROM attendantelectrical WHERE attendance_date = '$search_date'";
$result_search = mysqli_query($conn, $sql_search);
$search_count = mysqli_fetch_assoc($result_search)['total'];

$sql_search_present = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$search_date' AND status = 'Active'";
$result_search_present = mysqli_query($conn, $sql_search_present);
$search_present = mysqli_fetch_assoc($result_search_present)['count'];

$sql_search_absent = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$search_date' AND status = 'A'";
$result_search_absent = mysqli_query($conn, $sql_search_absent);
$search_absent = mysqli_fetch_assoc($result_search_absent)['count'];

$sql_search_late = "SELECT COUNT(*) as count FROM attendantelectrical WHERE attendance_date = '$search_date' AND status = 'P'";
...
$sql_total_all = "SELECT COUNT(*) as total FROM attendantelectrical";
...
function getYearStats($conn, $year, $shift = null, $status = null) {
    $sql = "SELECT COUNT(*) as total FROM attendantelectrical WHERE year = '$year'";
...
$yearly_totals = [];
$sql_yearly_total = "SELECT year, COUNT(DISTINCT student_id) as total FROM attendantelectrical GROUP BY year";
...
$sql_morning = "SELECT year, COUNT(*) as total FROM attendantelectrical WHERE shift = 'morning' AND status = 'Active' GROUP BY year";
...
$sql_evening = "SELECT year, COUNT(*) as total FROM attendantelectrical WHERE shift = 'Evening' AND status = 'Active' GROUP BY year";
...
<body>
    <div class="container">
        ...
        <div class="main-content">
            <div class="top-bar">
                <div class="page-title"><h2>📊   Attendance Management Electrical Engineering</h2></div>
                                ...
                                <?php while($row = mysqli_fetch_assoc($result)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href='delete_Attendance.php?id=<?php echo $row['id']; ?>' class='btn btn-danger btn-sm' onclick='return confirm("Are you sure you want to delete this record?")'><i class='fas fa-trash'></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
            ...
            <div class="form-container">
                ...
                <form action="processattcivil.php" method="POST" class="form-body" style="padding: 25px;">
                    <div class="mb-3">
