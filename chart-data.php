...
function getTeachersBySubject($conn) {
    // $sql = "SELECT subject, COUNT(*) as total FROM teachers WHERE subject IS NOT NULL AND subject != '' GROUP BY subject ORDER BY total DESC";
    $sql = "SELECT department as subject, COUNT(*) as total FROM teachers WHERE department IS NOT NULL AND department != '' GROUP BY department ORDER BY total DESC";
    $result = mysqli_query($conn, $sql);
...
$data['business'] = countWithCondition($conn, 'students', 'college', 'Business Science') + countWithCondition($conn, 'students', 'college', 'Business Science');
