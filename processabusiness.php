...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $check_sql = "SELECT id FROM attendantbusiness
                      WHERE student_id = ? 
                      AND attendance_date = ? 
                      AND subject = ?
                      AND year = ?
    ...
    if (!empty($errors)) {
        ...
        header("Location: attBusiness.php");
        exit();
    }
    
    // === INSERT RECORD (using correct table name: attendantit) ===
    $sql = "INSERT INTO attendantbusiness (student_id, attendance_date, subject, year, shift, status, note) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        error_log("SUCCESS: Inserted record ID: $record_id into attendantbusiness");
